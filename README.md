...
「全員」公開のため、デプロイ URL を知っていれば誰でも API を叩ける。
気になる場合は `gas/Code.gs` の `API_TOKEN` と `index.html` の `API_TOKEN` に
同じ合言葉を設定する。ただし `index.html` は公開されるのでトークンも
ソースを見れば読める。本気の認証が要るならリポジトリを private にするか、
GAS のアクセス権を「自分のみ」にして Google アカウント認証に切り替える。
